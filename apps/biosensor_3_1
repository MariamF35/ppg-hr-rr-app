...
# === Tkinter UI ===
def run_analysis():
    files = filedialog.askopenfilenames(title="Select _Signals.csv files", filetypes=[("CSV Files", "*.csv")])
    if not files:
        return

    try:
        max_readings = int(entry_readings.get())
    except ValueError:
        messagebox.showerror("Error", "Enter a valid number of readings")
        return

    text_results.delete("1.0", tk.END)

    for file in files:
        base = file.replace("_Signals.csv", "")
        results = analyze_file(base, max_readings=max_readings)
        text_results.insert(tk.END, f"Results for {base}:\n{results}\n\n")

    messagebox.showinfo("Done", "Analysis complete. Results saved to analysis_results.csv")

# === GUI Setup ===
root = tk.Tk()
root.title("PPG Signal Analysis")

tk.Label(root, text="Max readings to load:").pack(pady=5)
entry_readings = tk.Entry(root)
entry_readings.insert(0, "6000")
entry_readings.pack(pady=5)

btn_run = tk.Button(root, text="Select File(s) & Run Analysis", command=run_analysis)
...
1. working well
2. Problem faced - the directory that is opening is displaying all the Signals, Numerics, Breaths files all at once...user may choose wrong file...
3. Chnge to make - the program shd create another pop up screen displaying all Signal files only
